...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['avatar_url']) && $_FILES['avatar_url']['error'] === 0) {
        ...
        if ($img_size > 2097152) {
            ...
            if (in_array($img_ex_lc, $allowed_exs)) {
                ...
                if (move_uploaded_file($tmp_name, $img_upload_path)) {
                    ...
                    $query = "UPDATE users 
                                SET fullname = :fullname, email = :email, phone_number = :phone_number, user_address = :user_address, state = :state, about = :about, facebook = :facebook, avatar_url = :avatar_url
        ...
        $query = "UPDATE users 
                    SET fullname = :fullname, email = :email, phone_number = :phone_number, user_address = :user_address, state = :state, about = :about, facebook = :facebook
                    WHERE user_id = :user_id";
        $stmt = $conn->prepare($query);
        $stmt->bindParam(':fullname', $fullname);
        $stmt->bindParam(':email', $email);
